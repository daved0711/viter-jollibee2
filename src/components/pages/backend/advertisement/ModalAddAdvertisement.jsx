...
const ModalAddAdvertisement = ({ itemEdit }) => {
  ...
  const mutation = useMutation({
    mutationFn: (values) =>
      queryData(
        itemEdit
          ? `/v2/advertisement/${itemEdit.advertisement_aid}`
          : "/v2/advertisement",
        itemEdit ? "PUT" : "POST",
        values
      ),
  ...
  const initVal = {
    advertisement_aid: itemEdit ? itemEdit.advertisement_aid : "",
    advertisement_image: itemEdit ? itemEdit.advertisement_image : "",
    advertisement_title: itemEdit ? itemEdit.advertisement_title : "",
  };
      ...
      <ModalWrapper>
        <div className="modal-side absolute top-0 right-0 bg-primary h-[100dvh] w-[300px] border-l border-line">
          <div className="modal-header p-4 flex justify-between items-center">
            <h5 className="mb-0">Add Advertisements</h5>
              ...
              <Formik
                ...
                onSubmit={async (values) => {
                  mutation.mutate({
                    ...values,
                    advertisement_image:
                      (itemEdit?.advertisement_image === "" && photo) ||
                      (!photo && "") ||
                      (photo === undefined && "") ||
                      (photo && itemEdit?.advertisement_image !== photo?.name)
                        ? photo?.name || ""
